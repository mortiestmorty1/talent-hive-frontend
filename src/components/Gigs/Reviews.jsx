...
import SkillBasedReviews from "../SkillBasedReviews";
  ...
  useEffect(() => {
    if (gigData) {
      ...
      (gigData.reviews || []).forEach((r) => {
        avgRating += r.overallRating || r.rating || 0;
      });
      setAverageRatings(
        (avgRating / Math.max(1, (gigData.reviews || []).length)).toFixed(1)
      );
    }
  }, [gigData]);
              ...
              <div className="flex gap-3 border-t pt-6" key={review.id}>
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <FaStar
                          key={star}
                          className={`cursor-pointer ${
                            (review.overallRating || review.rating) >= star
                              ? "text-yellow-400"
                              : "text-gray-300"
                          } `}
                        />
                  ...
                  <div className="text-xs text-gray-600 flex gap-4">
                    {typeof review.communicationRating === "number" && (
                      <span>Comm: {review.communicationRating}</span>
                    )}
                    {typeof review.timelinessRating === "number" && (
                      <span>Time: {review.timelinessRating}</span>
                    )}
                    {typeof review.qualityRating === "number" && (
                      <span>Quality: {review.qualityRating}</span>
                    )}
                    {typeof review.skillSpecificRating === "number" && (
                      <span>Skill: {review.skillSpecificRating}</span>
                    )}
                    {review.skillCategory && (
                      <span>Category: {review.skillCategory}</span>
                    )}
